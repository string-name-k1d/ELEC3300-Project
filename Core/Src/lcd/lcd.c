...
__forceinline void LCD_Write_Cmd ( uint16_t usCmd )
{
...
__forceinline void LCD_Write_Data ( uint16_t usData )
{
...
__forceinline uint16_t LCD_Read_Data ( void )
{
...
void LCD_DrawStringN ( uint16_t usC, uint16_t usP, const char * pStr, uint8_t n)
{
	while ( * pStr != '\0' && n-- > 0)
	{
		if ( ( usC - LCD_DispWindow_Start_COLUMN + WIDTH_EN_CHAR ) > LCD_DispWindow_COLUMN )
